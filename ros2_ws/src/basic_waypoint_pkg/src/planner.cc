  ...
  mav_trajectory_generation::Trajectory * trajectory)
{
  ...
  // 1) Get ordered list of waypoint names.
  std::vector<std::string> waypoint_names;
  if (!node_->has_parameter("waypoint_names")) {
    node_->declare_parameter<std::vector<std::string>>("waypoint_names", std::vector<std::string>{});
  }
  (void)node_->get_parameter("waypoint_names", waypoint_names);

  // 2) For each waypoint, declare+read pos/vel/acc and add only existing constraints.
  for (const auto & wp : waypoint_names) {
    mav_trajectory_generation::Vertex middle(dimension);

    const std::string base = "waypoints." + wp + ".";

    // Declare with empty defaults so YAML overrides can populate them.
    if (!node_->has_parameter(base + "pos")) {
      node_->declare_parameter<std::vector<double>>(base + "pos", std::vector<double>{});
    }
    if (!node_->has_parameter(base + "vel")) {
      node_->declare_parameter<std::vector<double>>(base + "vel", std::vector<double>{});
    }
    if (!node_->has_parameter(base + "acc")) {
      node_->declare_parameter<std::vector<double>>(base + "acc", std::vector<double>{});
    }

    std::vector<double> pos, vel, acc;
    (void)node_->get_parameter(base + "pos", pos);
    (void)node_->get_parameter(base + "vel", vel);
    (void)node_->get_parameter(base + "acc", acc);

    bool added_any_constraint = false;

    // Position constraint (if provided)
    if (!pos.empty()) {
      if (pos.size() == static_cast<size_t>(dimension)) {
        Eigen::Vector3d p(pos[0], pos[1], pos[2]);
        middle.addConstraint(mav_trajectory_generation::derivative_order::POSITION, p);
        added_any_constraint = true;
      } else {
        RCLCPP_WARN(node_->get_logger(),
          "Waypoint '%s': pos has size %zu (expected %d). Ignoring pos.",
          wp.c_str(), pos.size(), dimension);
      }
    }

    // Velocity constraint (if provided)
    if (!vel.empty()) {
      if (vel.size() == static_cast<size_t>(dimension)) {
        Eigen::Vector3d v(vel[0], vel[1], vel[2]);
        middle.addConstraint(mav_trajectory_generation::derivative_order::VELOCITY, v);
        added_any_constraint = true;
      } else {
        RCLCPP_WARN(node_->get_logger(),
          "Waypoint '%s': vel has size %zu (expected %d). Ignoring vel.",
          wp.c_str(), vel.size(), dimension);
      }
    }

    // Acceleration constraint (if provided)
    if (!acc.empty()) {
      if (acc.size() == static_cast<size_t>(dimension)) {
        Eigen::Vector3d a(acc[0], acc[1], acc[2]);
        middle.addConstraint(mav_trajectory_generation::derivative_order::ACCELERATION, a);
        added_any_constraint = true;
      } else {
        RCLCPP_WARN(node_->get_logger(),
          "Waypoint '%s': acc has size %zu (expected %d). Ignoring acc.",
          wp.c_str(), acc.size(), dimension);
      }
    }

    // Only push if at least one constraint exists (usually pos is present).
    if (added_any_constraint) {
      vertices.push_back(middle);
    } else {
      RCLCPP_WARN(node_->get_logger(),
        "Waypoint '%s': no constraints found (pos/vel/acc all empty). Skipping.",
        wp.c_str());
    }
  }
  // ~~~~ end solution
