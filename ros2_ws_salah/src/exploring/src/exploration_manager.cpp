...
ExplorationManager::ExplorationManager()
    : Node("exploration_manager"), min_frontier_size_(5),
      ...
      los_short_range_threshold_(6.0), blacklist_max_size_(50) {
...
void ExplorationManager::blacklist_callback(
  ...
  // Sliding-window: evict oldest entry before adding a new one when at capacity
  if (blacklist_max_size_ > 0 &&
      static_cast<int>(blacklisted_goals_.size()) >= blacklist_max_size_) {
    RCLCPP_DEBUG(this->get_logger(),
                 "Blacklist at capacity (%d). Evicting oldest entry: (%.2f, %.2f, %.2f)",
                 blacklist_max_size_,
                 blacklisted_goals_.front().x(),
                 blacklisted_goals_.front().y(),
                 blacklisted_goals_.front().z());
  ...
  RCLCPP_INFO(
      this->get_logger(),
      "Blacklisted goal received: (%.2f, %.2f, %.2f). Blacklist size: %zu/%d",
      msg->point.x, msg->point.y, msg->point.z,
      blacklisted_goals_.size(), blacklist_max_size_);
}
...
void ExplorationManager::handle_get_goal(
  ...
  // 2.5. Compute blended "forward" vector.
  //
  // Two signals are combined:
  //   * Visual vector   (0.7 weight): drone -> centroid of visible frontiers.
  //     Reacts to local topology -- steers into branches, follows passages.
  //   * Expansion vector (0.3 weight): drone -> away from entrance / start.
  //     Prevents the "infinite frontier" retreat: when inside frontiers thin
  //     out, the massive frontier cluster near the cave mouth would otherwise
  //     pull the centroid backward.  The expansion component keeps a gentle
  //     "stay deep" bias independent of what the sensor currently sees.

  // --- Visual vector ---
  octomap::point3d frontier_centroid(0.0f, 0.0f, 0.0f);
  if (!downsampled.empty()) {
    for (const auto &pt : downsampled) {
      frontier_centroid += pt;
    }
    frontier_centroid /= static_cast<float>(downsampled.size());
  } else {
    frontier_centroid = drone_pos; // will fall back to expansion_vector below
  }
  octomap::point3d visual_vector = frontier_centroid - drone_pos;
  if (visual_vector.norm() > 1e-3f) {
    visual_vector.normalize();
  }

  // --- Expansion vector (away from entrance / start position) ---
  // entrance_pos_ is captured on the very first goal request (already exists).
  octomap::point3d expansion_vector = drone_pos - entrance_pos_;
  if (expansion_vector.norm() > 1e-3f) {
    expansion_vector.normalize();
  } else {
    // Drone is still at the entrance -- fall back to pure visual signal.
    expansion_vector = visual_vector;
  }

  // --- Blend ---
  constexpr float kVisualWeight    = 0.7f;
  constexpr float kExpansionWeight = 0.3f;
  octomap::point3d forward_vector =
      (visual_vector * kVisualWeight) + (expansion_vector * kExpansionWeight);
  if (forward_vector.norm() < 1e-3f) {
    // Degenerate: vectors cancelled -- use expansion alone
    forward_vector = expansion_vector;
  } else {
    forward_vector.normalize();
  }

  RCLCPP_DEBUG(this->get_logger(),
               "Fwd: (%.2f,%.2f,%.2f) visual: (%.2f,%.2f,%.2f) expansion: (%.2f,%.2f,%.2f)",
               forward_vector.x(), forward_vector.y(), forward_vector.z(),
               visual_vector.x(), visual_vector.y(), visual_vector.z(),
               expansion_vector.x(), expansion_vector.y(), expansion_vector.z());
  ...
  for (auto &cand : candidates) {
    ...
    // Calculate utility using the dynamic forward vector
    cand.utility = evaluate_candidate(cand.position, drone_pos, forward_vector);
    num_evaluated++;
...
double ExplorationManager::evaluate_candidate(const octomap::point3d &candidate,
                                              ...
                                              const octomap::point3d &forward_vector) {
  octomap::point3d vec_to_candidate = candidate - drone_pos;
  double dz = vec_to_candidate.z();
  double dist_3d = vec_to_candidate.norm();
  ...
  // --- Dynamic forward bias (replaces hardcoded "negative-x = forward") ---
  // Project the vector-to-candidate onto the centroid-based forward direction.
  double forward_projection = vec_to_candidate.dot(forward_vector);

  if (forward_projection > 0.1) {
    // Candidate lies in the "forward" hemisphere: boost utility
    // Bonus scales from 1x to 5x depending on forward distance (caps out at 20m)
    double forward_bonus = 1.0 + std::min(4.0, forward_projection / 5.0);
    utility *= forward_bonus;
  } else {
    // Backward or purely lateral: penalize heavily to disfavour backtracking
    utility *= 0.1;
  }

  // Lateral penalty: penalise large sideways deviation relative to forward progress.
  // Compute the component perpendicular to the forward direction.
  octomap::point3d parallel_component   = forward_vector * static_cast<float>(forward_projection);
  octomap::point3d perp_component       = vec_to_candidate - parallel_component;
  double lateral_distance               = perp_component.norm();
  double fwd_abs                        = std::abs(forward_projection);
  double lateral_ratio = (fwd_abs > 1.0) ? lateral_distance / fwd_abs : lateral_distance;
